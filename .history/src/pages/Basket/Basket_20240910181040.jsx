...
const Basket = ({ handleRemoveItem }) => {
    ...
    let quantity = retArrayBasket.map((acc, item) =>  acc.cost * acc.quantity, 0)
    console.log(retBasket)
      return (
        <div className="cart">
          ...
          <p className="total-price">Общая стоимость: {retArrayBasket.map((acc, item) =>  acc.cost * acc.quantity, 0)} руб.</p>
        </div>
      );
    };
